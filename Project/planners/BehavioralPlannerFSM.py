...
class BehavioralPlannerFSM(object):
    ...
    def get_closest_waypoint_goal(
        self, ego_state: State, map: carla.Map, lookahead_distance: float
    ):
        waypoint_0 = map.get_waypoint(ego_state.location)

        if (
            self._active_maneuver == Maneuver.DECEL_TO_STOP
            or self._active_maneuver == Maneuver.STOPPED
        ):
    ...
    def get_look_ahead_distance(self, ego_state: State):
        ...
        look_ahead_distance = velocity_mag**2 / (1 * 2)  # <- calculate value

        # Cap look_ahead_distance in range [_lookahead_distance_min, _lookahead_distance_max]
        look_ahead_distance = min(
            max(look_ahead_distance, self._lookahead_distance_min),
            self._lookahead_distance_max,
        )
    ...
    def state_transition(
        ...
        if self._active_maneuver == Maneuver.FOLLOW_LANE:
            if is_goal_junction:
                ...
                goal.location.y += (
                    np.sin(ang) * self._stop_line_buffer
                )  # <- calculate appropiate coordinate
                ...
                goal.velocity.x = self._speed_limit * np.cos(
                    goal.rotation.yaw
                )  # <- caluclate value
                goal.velocity.y = self._speed_limit * np.sin(
                    goal.rotation.yaw
                )  # <- caluclate value
                goal.velocity.z = 0.0  # <-  caluclate value
    ...
    def get_goal(self, ego_state: State, map: carla.Map):
        ...
        is_goal_junction, goal_wp = self.get_closest_waypoint_goal(
            ego_state, map, look_ahead_distance
        )

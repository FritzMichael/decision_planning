...
class MotionPlanner(object):
    ...
    def generate_offset_goals(self, goal_state : State):
        ...
        for i in range(self._num_paths):
            ...
            goal_offset.location.x += np.cos(yaw_plus_90) * offset # calculates this
            goal_offset.location.y += np.sin(yaw_plus_90) * offset # calculates this
    ...
    def get_best_spiral_idx(self, spirals : List[List[PathPoint]], obstacles : List[State], goal_state : State):
        ...
        if len(collisions) == len(spirals):
            print("All spirals collide", len(spirals))
